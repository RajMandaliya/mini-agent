...
pub fn parse_openai_completion(json: &Value) -> Result<Completion, AgentError> {
    ...
    let choice = json
        ...
        .ok_or_else(|| {
            AgentError::invalid(
                &provider,
                "response missing 'choices' array — the API may have returned an error body",
            )
        })?;
    ...
    if let Some(calls) = message.get("tool_calls").and_then(|v| v.as_array()) {
        for (i, call) in calls.iter().enumerate() {
            ...
            let function = call.get("function").ok_or_else(|| {
                AgentError::invalid(
                    &provider,
                    format!("tool_call[{i}] is missing 'function' field"),
                )
            })?;
            let name = function
                .get("name")
            ...
            let args_raw = function.get("arguments").ok_or_else(|| {
                AgentError::invalid(
                    &provider,
                    format!("tool_call[{i}] function '{name}' is missing 'arguments'"),
                )
            })?;
